...
exports = module.exports = function (db) {
    ...
    exports.getActors = function (req, res) {
        ...
        db.getIndexedNodes('node_auto_index', 'type', 'actor',
            ...
            // the attributes of the actor (like title, description) are stored inside
            ...
            logger.debug('Successfully loaded %d actor.', actors.length);
            res.send(actors);
        });
    };
    ...
    exports.getActor = function (req, res) {
        ...
        db.getIndexedNode('node_auto_index', 'id', id, function (err, node) {
            if (err) {
                ...
                logger.debug('#%s could not be found.', id);
                return res.status(404).send();
            }
            logger.debug('Found actor#%s with title: %s', id, node.data.title);
            res.send(node.data);
        });
    };
